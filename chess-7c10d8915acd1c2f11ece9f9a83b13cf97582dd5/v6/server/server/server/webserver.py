...
def load_file(path:str) -> str:
    ...
    '''The load_file function attempts to read the content of a file with given path and returns it as a string'''

    with open("/".join(re.split('/|\\\\',os.path.dirname(__file__))[:-1])+path,'r') as content:
...
def render_page(path:str, args:dict) -> str:
    ...
    content = ''
    try:
        with open("/".join(re.split('/|\\\\',os.path.dirname(__file__))[:-1])+path,'r') as page:
            content = '\n'.join(page.readlines())
        to_eval = re.findall('{{[^{}]*}}',content)
        for var in to_eval:
            OUTPUT = {}
